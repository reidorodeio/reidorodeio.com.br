                    ...
                    <div class="single-payment-wrap">
                        <div class="row mb-4">
                            ...
                            <div class="col-lg-4 text-lg-end">
                                @if ($ticket_object->status == 1)
                                    <button class="btn btn-warning btn-sm pull-end mt-2"> {{ __('Aberto') }}</button>
                                @elseif($ticket_object->status == 2)
                                    <button type="button" class="btn btn-success btn-sm pull-end"> {{ __('Respondido') }}
                                    </button>
                                @elseif($ticket_object->status == 3)
                                    <button type="button" class="btn btn-info btn-sm pull-end">
                                        {{ __('Resposta do Usuário') }} </button>
                                @elseif($ticket_object->status == 9)
                                    <button type="button" class="btn btn-danger btn-sm pull-end"> {{ __('Fechado') }}
                                    </button>
                                @endif
                                <a href="{{ route('ticket.close', $ticket_object->ticket) }}"
                                    class="btn btn-danger btn-sm pull-end make-close-support mt-2">{{ __('Marcar como Fechado') }}</a>
                            </div>
                        </div>
                        ...
                        <div class="row">
                            <div class="contact-inner">
                                <form method="POST" action="{{ route('store.user.reply', $ticket_object->ticket) }}"
                                    accept-charset="UTF-8" class="form-horizontal form-bordered">
                                    @csrf
                                    <div class="col-lg-12">
                                        <div class="single-input-inner style-border">
                                            <span class="input-group-text">{{ __('Resposta') }}:</span>
                                            <textarea class="border-0 bg-black-5" name="comment" cols="30" rows="10"></textarea>
                                        </div>
                                        <button type="submit" class="btn btn-base mt-3">{{ __('Publicar') }}</button>
                                    </div>
                                </form>
                            </div>
                        </div>
