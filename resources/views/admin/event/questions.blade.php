...
@section('content')
    <div class="card">
        <div class="card-header">
            <h4 class="card-title text-end"><a href="{{ route('admin.all.matches') }}"
                    class="btn btn-sm s7__btn-dark @if (Request::routeIs('admin.all.matches')) active @endif">
                    <i class="las la-arrow-left"></i> {{ __('Go Back') }}
                </a>
                @can('admin-save-question')
                    <button type="button" data-bs-toggle="modal" data-bs-target="#questionsAddModel"
                            ...
                            <td>
                                @can('admin-update-question')
                                    <button type="button" class="btn s7__btn-secondary btn-sm edit_qus_button"
                                        title="Edit Question" data-bs-toggle="modal" data-bs-target="#editQuestionsModal"
                                        data-act="Edit" data-name="{{ $data->question }}"
                                ...
                                @can('admin-view-option')
                                    <a href="{{ route('admin.view.option', $data->id) }}"class="btn s7__btn-info btn-sm"><i
                                            class="fa-solid fa-bars-staggered"></i> {{ __('Option') }}</a>
                                @endcan
                            </td>
                        </tr>
                    @endforeach
    ...
    <div class="modal fade" id="questionsAddModel" tabindex="-1" role="dialog" aria-labelledby="ModalLabel"
        aria-hidden="true">
        <div class="modal-dialog">
            <div class="modal-content">
                ...
                {!! Form::open(['route' => 'admin.save.question', 'method' => 'POST']) !!}
                <div class="modal-body">
                        ...
                        <div class="col-sm-6">
                            ...
                            <div class="input-group date">
                                <input type="text" id="pickupDate" class="form-control datepicker-field"
                                    data-language="pt" data-position='bottom left' name="end_date" autocomplete="off">
                    ...
                    <div class="form-group">
                        <p class="text-danger">{{ __('Date & Time must be set Before') }}
                            {{ date('d M, Y - h:i A', strtotime($match_id->end_date)) }}</p>
                    </div>
                </div>
    ...
    <div class="modal fade" id="editQuestionsModal" tabindex="-1" role="dialog" aria-labelledby="ModalLabel"
        aria-hidden="true">
        <div class="modal-dialog">
            <div class="modal-content">
                <div class="modal-header">
                ...
                <div class="modal-body">
                    ...
                    <div class="form-group row">
                        <div class="col-sm-6">
                            ...
                            <div class="input-group date">
                                <input type="text" id="pickupDate"
                                    class="form-control edit_qus_date datepicker-field2" data-language="pt"
                        ...
                        <div class="col-md-6">
                            <label><strong>{{ __('End Time') }}</strong></label>
                            {!! Form::time('end_time', date('H:i'), ['placeholder' => 'end_time', 'class' => 'form-control edit_qus_time']) !!}
                        </div>
                    </div>
                    <div class="form-group">
                        <p class="text-danger">{{ __('Date & Time must be set Before') }}
