            ...
            <table class="table s7__table">
                ...
                <tbody>
                    @foreach ($matches as $key => $data)
                        <tr>
                            <td>{{ $key + 1 }}</td>
                            <td><img src="{{ asset('public/images/match/' . $data->team_1_image) }}" alt="img">
                                {{ $data->team_1 }} <label class="badge badge-info"> {{ __('VS') }} </label> <img
                            ...
                            <td>
                                <label
                                    class="badge bg-{{ $data->status == 2 ? 'danger' : 'success' }}">{{ $data->status == 2 ? 'Closed' : 'Active' }}</label>
                            </td>
                            <td>
