...
@section('content')
                ...
                <tbody>
                    @foreach ($questions as $key => $data)
                        <tr>
                            ...
                            <td>
                                @if (!is_null($data->match) && !is_null($data->match->team_1_image))
                                    <img src="{{ asset('public/images/match/' . $data->match->team_1_image) }}"
                                        alt="img">
                                @endif
                                @if (!is_null($data->match))
                                    {{ $data->match->team_1 }} <label class="badge bg-info"> {{ __('VS') }} </label>
                                    @if (!is_null($data->match->team_2_image))
                                        <img src="{{ asset('public/images/match/' . $data->match->team_2_image) }}"
                                            alt="img">
                                    @endif
                                    {{ $data->match->team_2 }}
                                @endif
                            ...
                            <td>
                                @can('admin-awaiting-winner-userlist')
                                    <a href="{{ route('admin.awaiting.winner.userlist', $data->id) }}"
                                        class="btn s7__btn-primary btn-sm">{{ $data->totalInvestor() }}</a>
                                @endcan
                            </td>
                            <td>
                                @if ($data->result == 1)
                                    @can('admin-view-option-endtime')
                                        <a href="{{ route('admin.view.option.endtime', $data->id) }}"
                                            class="btn s7__btn-success btn-sm" title="View Winner">
                                ...
                                @else
                                    @can('admin-view-option-endtime')
                                        <a href="{{ route('admin.view.option.endtime', $data->id) }}"
                                            class="btn s7__btn-secondary btn-sm" title="Select Winner">
                                            <i class="las la-trophy"></i>
                                        </a>
                                    @endcan
                                    @can('admin-update-question')
                                        <button type="button" class="edit_awaiting_ques btn s7__btn-info btn-sm"
                                            title="Update Question" data-bs-toggle="modal" data-bs-target="#EditAwaitingModal"
                                            data-act="Edit"
                                            data-matchenddate="{{ !is_null($data->match) ? date('d M, Y H:i', strtotime($data->match->end_date)) : '' }}"
                                            data-name="{{ $data->question }}"
                                            data-datetime="{{ date('H:i', strtotime($data->end_time)) }}"
                                            data-enddate="{{ date('d/m/Y', strtotime($data->end_time)) }}"
                                    ...
                                    @can('admin-refundbetinvest')
                                        <button type="button" class="refund_bet btn s7__btn-danger btn-sm"
                                            title="Refund Prediction Amount" data-bs-toggle="modal"
                                            data-bs-target="#refundMyModal" data-id="{{ $data->id }}"
                                            data-mid="{{ $data->match_id }}">
    ...
    <div class="modal fade" id="EditAwaitingModal" tabindex="-1" role="dialog" aria-labelledby="ModalLabel"
        aria-hidden="true">
        <div class="modal-dialog">
            <div class="modal-content">
                ...
                {!! Form::open(['route' => 'admin.update.question', 'method' => 'POST']) !!}
                    ...
                    <div class="form-group row">
                        <div class="col-sm-6">
                            ...
                            <div class="input-group date">
                                <input type="text" id="pickupDate"
                                    class="form-control edit_awaiting_date datepicker-field" data-language="pt"
                        ...
                        <div class="col-md-6">
                            ...
                            {!! Form::time('end_time', date('H:i'), [
                                ...
                                'class' => 'form-control edit_awaiting_time',
                            ]) !!}
                        </div>
                    </div>
                    <div class="form-group">
                        <p class="text-danger">{{ __('This Date & Time will working  Before') }} <span
                                class="awaiting_match_end_date"></span></p>
                    </div>
                    <div class="form-group">
    ...
    <div class="modal fade" id="refundMyModal" tabindex="-1" role="dialog" aria-labelledby="ModalLabel"
        aria-hidden="true">
        <div class="modal-dialog">
            <div class="modal-content">
                <div class="modal-header">
                ...
                {!! Form::open(['route' => 'admin.refundBetInvest', 'method' => 'POST']) !!}
                <div class="modal-body">
                    <div class="form-group">
