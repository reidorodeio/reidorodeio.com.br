            ...
            <h4 class="card-title">
                <div class="row">
                    ...
                    <div class="col-md-8 text-end">
                        <p>
                            @if (!is_null($betQuestion->match))
                                <p>
                                    @if (!is_null($betQuestion->match->team_1_image))
                                        <img src="{{ asset('public/images/match/' . $betQuestion->match->team_1_image) }}"
                                            alt="img">
                                    @endif
                                    {{ $betQuestion->match->team_1 }} <label
                                        class="badge bg-info">{{ __('VS') }}</label>
                                    @if (!is_null($betQuestion->match->team_2_image))
                                        <img src="{{ asset('public/images/match/' . $betQuestion->match->team_2_image) }}"
                                            alt="img">
                                    @endif
                                    {{ $betQuestion->match->team_2 }}
                                </p>
                            @else
                                <p>{{ __('Match details not available') }}</p>
                            @endif
                        </p>
                    </div>
                </div>
                    ...
                    @if (count($betInvest) > 0)
                        @foreach ($betInvest as $key => $data)
                            <tr>
                                ...
                                <td><strong
                                        class="text-success">{{ number_format($data->invest_amount, $general->decimal) }}
                                        {{ $general->currency }} </strong></td>
                                <td><strong
                                        class="text-danger">{{ number_format($data->return_amount, $general->decimal) }}
                                        {{ $general->currency }} </strong></td>
                                ...
                                <td>
                                    ...
                                    @elseif($data->status == 0)
                                        @can('admin-refundBetInvestSingleUser')
                                            <button type="button" class="btn s7__btn-primary btn-sm refund_prelist_bet"
                                                title="Refund Prediction Amount" data-bs-toggle="modal"
                                                data-bs-target="#refundPrelistMyModal" data-act="Refund"
                                                data-id="{{ $data->id }}">
                                                <i class="ti-na"></i>
                                            </button>
                                        @endcan
