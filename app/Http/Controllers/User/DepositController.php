...
class DepositController extends Controller
{
    ...
    public function userDataUpdate($data)
    {
        if ($data->status == 0) {
            ...
            createTransaction("Depósito via " . $data->gateway->name, $data->amount, $user->balance, $user->balance + $data->amount, 1);
            ...
            Session::flash('success', 'Depositado ' . $data->amount . ' ' . $gnl->currency . ' com sucesso');

            $adminNotification = new Notification();
            $adminNotification->user_id = $user->id;
            $adminNotification->title = 'Depósito bem-sucedido via ' . $data->gateway->name;
        ...
        return redirect()->route('deposit.index')->with('alert', 'Algo deu errado');
    }
    ...
    public function depositPayNow(Request $request)
    {
        ...
        try {
            ...
            if ($gatewayId == 16) {
                ...
                $client = Client::create($gateway->gateway_key_two);
                $correlationID = Str::uuid()->toString();

                try {
                    $customer = $client->customers()->getOne($user->cpf);
                } catch (ApiErrorException $e) {
                    if ($e->getMessage() === 'Cliente não encontrado') {
                        $customer = [
                            "name" => $user->name,
                            "taxID" => $user->cpf,
                ...
                Deposit::where('id', $deposit_table->id)->update(['identifier' => $correlationID]);

                $charge = [
                    "value" => intval($total_amount * 100),
                    "correlationID" => $correlationID,
                    "customer" => $customer,
                ];
                ...
                return view('user.deposit.payment_views.openpix', $deposit_table, compact('track', 'page_title', 'result'));
            }
            ...
            if ($gatewayId > 99) {
                ...
                $adminNotification->title = 'Solicitação de depósito enviada com sucesso via ' . $gateway->name;
                ...
                return redirect()->route('deposit.index')->with('success', 'Solicitação de depósito enviada com sucesso!');
            }

            return redirect()->route('deposit.index')->with('alert', 'Algo deu errado, por favor tente novamente mais tarde.');
        } catch (\Exception $e) {
    ...
    {
        $data['page_title'] = "Registro de Depósitos";
