...
function levelCommision($id, $amount)
{
    ...
    while ($usr != "" && $usr != "0" && $i <= $level) {
        ...
        createTransaction('Parabéns, você recebeu uma comissão de indicação ' . $i . ' de ' . $user->name, $com, $user->balance, $new_bal, 5, $user->id);
        $user->save();
        ...
        send_email($user, "REFERRAL_COMMISSION", $shortCodes); // Removi o '@' antes de send_email
        $usr = $user->id;
        $i++;
    }
    return 0;
...
function send_email($user, $type, $shortCodes = null)
{
    ...
    $messageTemplate = $general->global_description;
    $message = shortCodeReplacer("{{name}}", $user->name, $messageTemplate);
