...
let innerGame = document.querySelector('#gaming_page_inner');
let container = document.createElement("div");
container.setAttribute("id", "container");
// document.body.append(container);
innerGame.append(container)
...
function resetGame() {
    bankValue = document.currentScript.getAttribute('data-balance');
    currentBet = 0;
    wager = 5;
    bet = [];
    numbersBet = [];
    ...
    for (i = 0; i < chipValues.length; i++) {
        ...
        chip.onclick = function () {
            ...
            if (cvi !== 4) {
                ...
                wagerAmount(chip.childNodes[0].innerText)
            } else {
...
function setBet(e, n, t, o) {
    clearBet();
    removeChips();
    var gameObj = JSON.parse(games);

    if (arguments[2] == 'outside_dozen') {
        // *3
        var parRate = gameObj.rate5;
        winBonusAmt(gameObj.rate5);
    } else if (arguments[2] == 'outside_oerb') {
        // *4
        var parRate = gameObj.rate4;
        winBonusAmt(gameObj.rate4);
    } else if (arguments[2] == 'outside_column') {
        // *3
        var parRate = gameObj.rate3;
        winBonusAmt(gameObj.rate3);
    } else if (arguments[2] == 'zero') {
        // *2
        var parRate = gameObj.rate2;
        winBonusAmt(gameObj.rate2);
    } else if (arguments[2] == 'inside_whole') {
        // *1
        var parRate = gameObj.rate1;
        winBonusAmt(gameObj.rate1);
    }
    ...
    payOut = parseFloat(wager) * parseFloat(parRate);

    $('#payAmount').val(payOut);
    if (wager > 0) {
...
function win(winningSpin, winValue, betTotal) {
    ...
    if (winValue > 0) {
        console.log(wager, winBonus, payOut);
        var item = {
            beting_amount: wager,
            win_bonus: winBonus,
            pay_out: payOut,
...
function winBonusAmt(val) {
    $('#winBonus').val(val);
    winBonus = val;
}

function wagerAmount(val) {
    $('#betAmout').val(val);
}
